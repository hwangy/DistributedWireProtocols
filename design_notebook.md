...
The size of the buffers being sent back and forth between the client and the server was rather similar between the two implementations (previous implementation vs gRPC), at least with respect to scaling. By scaling, we mean that both store the different components of the object in a similar fashion, and using gRPC resulted in a constant multiplicative factor scaling of the size of the buffer. This similarity in the size was because the way we originally implemented the code shared several similarities with the way that gRPC automatically sets up request and reply objects. However, as we will discuss, there is some overhead for the gRPC implementation, with respect to the size of the buffers.

When we implemented the code without gRPC, we set up request and response objects for each of the 5 methods we had to implement, plus login and logout request and response objects. When gRPC was used, gRPC automatically generated request and reply objects, which we equipped with the same type of functionality as in our original implementation. The number of request and reply objects, their methods, and their stored information were all very comparable. For this reason, the size of the buffers end up being similar between the two implementation with respect to scaling. 

We now note that the gRPC implementation did have some overhead in terms of the number of bytes used to encode request and reply objects (versus request and response objects in our Part 1 implementation). For example, the ``CreateAccountRequest`` object in our Part 1 implementation used 13 bytes, while the ``CreateAccountRequest`` object in our Part 2 gRPC implementation used 40 bytes. The gRPC implementation used more bytes because it provided a more complete encoding of what the request and response objects should be. gRPC has a sophisticated encoding of the Java objects that makes marshalling and unmarshalling easy. On the othe rhand, or wire protocol had a very precise specification of all of the components of the request and reponse objects, with very little overhead. The only component of the ``CreateAccountRequest`` object, for example, that could qualify as overhead would be the ``method``, because it is what is needed for marshalling and unmarshalling. Without gRPC, the marshalling and unmarshalling logic added a lot of implementation complexity.

As another example, let's compare the ``SendMessageRequest`` object between the Part 1 and Part 2 (gRPC) implementations. We compare the two when a short (23 character) and long (206 character) message is sent. For the Part 1 implementation, a 23 character message resulted in a 37 byte ``SendMessageRequest`` specification. A 206 character message resulted in a 220 byte ``SendMessageRequest`` specification. Both are 14 byte overheads. For the Part 2 gRPC implementation, a 23 character message resulted in a 74 byte ``SendMessageRequest`` specification. A 206 character message resulted in a 257 byte ``SendMessageRequest`` specification. Both are 51 byte overheads. We can observe that the length of the total request scales linearly in the size of the message for both implementations (Part 1 vs Part 2). From this we can deduce that if the length of the message dominates the length of the request (i.e. contributes to a significant portion of the length), then the overhead encountered by gRPC quickly becomes negligible.

### The complexity of the code

Using our reasoning about the size of the buffers above, we now speculate what this means for the compleixty of the code. Since the size of the buffers is rather comparable in terms of scaling, the code (without gRPC and with gRPC) will have a similar complexity ultimately.
...
Using gRPC also makes it significantly easier to modify the wire protocol being used. Before, with our wire protocol in Part 1, if we made a modification we had to change the parsing of arguments, number of arguments, and other aspects throughout the code. In short, refactoring the wire protocol could be time-consuming. When gRPC is used, refactoring the wire protcol is a lot easier and less components of the code needed to be modified.

### Performance differences

The two implementations (Part 1 without gRPC vs Part 2 with gRPC) have a very similar performance overall when run. This encompasses the fact that the same messages are sent back and forth between the two implementations. The client and server have the same behavior and follow the same structure in their implementation. The implementations do vary when gRPC is used or not based on the specifications gRPC and our wire protocol each needed. 

In terms of runtime efficiency differences, we observed that the gRPC implementation took longer to start up than the Part 1 implementation.